...
def l2_error(p_des: npt.NDArray[np.float64], p: npt.NDArray[np.float64]) -> float:
    """
...
def euler_error(R_des: npt.NDArray[np.float64], R_curr: npt.NDArray[np.float64]) -> float:
    """
...
def rodrigues_error(R_des: npt.NDArray[np.float64], R_curr: npt.NDArray[np.float64]) -> float:
    """
...
def quat_error(q_des: npt.NDArray[np.float64], q: npt.NDArray[np.float64]) -> float:
    """
...
def combined_error(
    ep: float,
    etheta: float,
    _lambda: float = 1e-6 / np.deg2rad(1) ** 2
) -> float:
    """
